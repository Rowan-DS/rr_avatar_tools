...
class RecRoomRoomieDiagnosticOperator(RecRoomRoomieMeshOperator):
...
class RR_OT_RoomieDiagnosticsFixUnappliedTransforms(RecRoomRoomieDiagnosticOperator):
    """"""

    bl_idname = "rr.roomie_diagnostics_fix_unapplied_transforms"
...
class RR_OT_RoomieDiagnosticsFixMissingArmatureModifier(
    RecRoomRoomieDiagnosticOperator
):
    """"""
    ...
    def get_body_type(cls, mesh):
        ...
        if check_prefix(mesh, "RM1_"):
            return "RM1"

        collections = mesh.users_collection

        if any(map(lambda x: check_prefix(x, "RM1_"), collections)):
            return "RM1"
    ...
    def get_rig(cls, mesh):
        ...
        if body_type in ("RM1",):
            return bpy.data.objects.get("Roomie_Skeleton")
    ...
    def can_fix(cls, context):
        def can_fix_(o):
            return cls.get_body_type(o) in ("RM1",)
...
class RR_OT_RoomieDiagnosticsLimitBonesPerVertex(RecRoomRoomieDiagnosticOperator):
    ...
    bl_idname = "rr.roomie_diagnostics_limit_bones_per_vertex"
...
class RR_OT_RoomieDiagnosticsFixNgons(RecRoomRoomieDiagnosticOperator):
    """"""

    bl_idname = "rr.roomie_diagnostics_fix_ngons"
    bl_label = "Fix Ngons"
    bl_options = {"REGISTER", "UNDO"}
...
class RR_OT_RoomieDiagnosticsCheckTriangleCount(RecRoomRoomieDiagnosticOperator):
    """"""

    bl_idname = "rr.roomie_diagnostics_check_triangle_count"
...
class RR_OT_RoomieDiagnosticsCheckBounds(RecRoomRoomieDiagnosticOperator):
    """"""

    bl_idname = "rr.roomie_diagnostics_check_bounds"
...
classes = (
    RR_OT_RoomieDiagnosticsFixUnappliedTransforms,
    RR_OT_RoomieDiagnosticsFixMissingArmatureModifier,
